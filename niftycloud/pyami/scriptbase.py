import os
import sys
from niftycloud.utils import ShellCommand, get_ts
import niftycloud
...
class ScriptBase(object):
    ...
    def notify(self, subject, body=''):
        niftycloud.utils.notify(subject, body)
    ...
    def mkdir(self, path):
        if not os.path.isdir(path):
            try:
                ...
                niftycloud.log.error('Error creating directory: %s' % path)
    ...
    def run(self, command, notify=True, exit_on_error=False, cwd=None):
        ...
        if self.last_command.status != 0:
            niftycloud.log.error('Error running command: "%s". Output: "%s"' % (command, self.last_command.output))
            if notify:
